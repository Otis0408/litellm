...
def simple_shuffle(
    ...
    for weight_by in ["weight", "rpm", "tpm"]:
        raw_weights = [m.get("litellm_params", {}).get(weight_by) for m in healthy_deployments]
        if all(raw_weight is None for raw_weight in raw_weights):
            continue
        weights = [1 if raw_weight is None else raw_weight for raw_weight in raw_weights]
